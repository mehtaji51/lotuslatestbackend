...
const {
  createNewCourse,
} = require('../../controllers/course/create-new-course.js');
const {
  updateCourseData,
} = require('../../controllers/course/update-course-content.js');
  ...
  try {
    ...
    if (error instanceof mongoose.Error.VersionError) {
      ...
      res.status(200).json({
        success: true,
        message:
          'Course updated successfully, but there was a version conflict. Refetch the latest data.',
...
router.get('/get-user-enrollments', async (req, res, next) => {
  try {
    ...
    const enrollments = await Enrollment.find({ learner: userId }).populate(
      'course'
    );
  ...
  try {
    ...
    if (result.nModified === 0) {
      return res
        ...
        .json({
          success: false,
          message: 'No enrollments found or no lessons were updated.',
        });
    }
    ...
    return res.status(200).json({
      success: true,
      message:
        'Lesson and associated grades removed from all relevant enrollments.',
    });
  } catch (error) {
  ...
  try {
    ...
    if (!institutionCode) {
      return res
        ...
        .json({
          success: false,
          message: 'Institution code not found for this user.',
        });
    }
    ...
    if (!enrollments.length) {
      return res.status(400).json({
        success: false,
        message:
          'User is already enrolled in all available courses for this institution.',
      });
    }
...
router.post('/remove-students-from-course', async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `${
        updatedStudents.filter(Boolean).length
...
router.post('/remove-student-from-course', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to remove student.' });
  }
});
...
router.post('/complete-lesson', async (req, res) => {
  try {
    ...
    if (!lesson) {
      return res
        .status(404)
        .json({ message: 'Lesson not found in the course' });
    }
...
router.get('/get-all-grades/:studentId', async (req, res) => {
  try {
    ...
    for (const enrollment of enrollments) {
      if (enrollment.lessonGrades && enrollment.lessonGrades.length > 0) {
        for (let grade of enrollment.lessonGrades) {
          ...
          if (
            lessonFromCourse &&
            lessonFromCourse.title !== grade.lessonTitle
          ) {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Error fetching grades' });
  }
});
  ...
  try {
    ...
    if (!fs.existsSync(tempDir)) {
      try {
        ...
        return res
          .status(500)
          .json({ success: false, message: 'Error creating temp directory' });
      }
    }
    ...
    for (const student of students) {
      try {
        ...
        return res
          ...
          .json({
            success: false,
            message: `Error processing student ${student.username}`,
          });
      }
    }
    ...
    output.on('close', () => {
      ...
      if (!fs.existsSync(zipFilePath)) {
        return res
          .status(500)
          .json({ success: false, message: 'ZIP file not found' });
      }
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Error downloading grades' });
  }
});
